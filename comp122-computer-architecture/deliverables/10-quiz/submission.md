...
What are the $N$ characteristics of a Universal Computer that differentiates it from a Turing machine
   ...
   1. infinite tape is transferred into limited memory                     <!-- response -->
   1. specialized control unit                                             <!-- response -->
   1. operating system is a special program                                <!-- response -->
   1. user-program is a generic program                                    <!-- response -->
...
The Process Status Diagram (PSD) is a specific Finite State Machine that models the execute flow of a process.  During it's lifetime a process goes through a number of states. Enumerate the names of each of these states and briefly describe something purpose of the state.
   ...
   1. Ready: prepared and waiting for its time on cpu                       <!-- response -->
   1. Running: Goes onto CPU time and is being executed                     <!-- response -->
   1. Interrupt: process is sent back to the Ready stage                    <!-- response -->
   1. Waiting: an error occured and trap occurred                           <!-- response -->
   1. Terminate: done!                                                      <!-- response -->
...
A Pushdown Automata (PDA) has more computational power than a Finite State Machine (FSM). What is the difference between the PDA and and FSM that provides the PDA with this additional power?
  ...
  ```response
  FSM has a finite amount of memory and therefore can only keep track of a small amount of information whereas the PDA has an unbounded amount of memory.
  ```
...
## User Memory
   ...
   1. Text segment contains instructions for programs          <!-- response -->
   1. Data segment contains static data                        <!-- response -->
   1. Heap segment contains dynamic data                       <!-- response -->
   1. Stack segment contains temporary data                    <!-- response -->
...
## Registers
   ...
   1.  `$v0`: Holds a value                                    <!-- response -->
   1.  `$sp`: Stack Pointer                                    <!-- response -->
   1.  `$zero`: always holds value zero                        <!-- response -->
   1.  `$a2`: passes arguments                                 <!-- response -->
   1.  `$t4`: holds data temporarily                           <!-- response -->
